...
# 	generated_columns = ['time', 'weekday', 'station_name', 'label', 'value']
	...
	def __init__(self):
		...
		self.batch_size = list(self.configuration.values[:,1])[0]
		self.epochs = list(self.configuration.values[:,2])[0]
	...
	def create_model(self):		
		...
		print(f"The number of INPUT nodes is {self.train_x.shape[1]}.")


		model.add(LSTM(self.train_x.shape[1], input_shape=(self.train_x.shape[1], self.train_x.shape[2]), return_sequences=True))
...
# 			print(y)
			...
			# Reshape from (288,X) to (1,288,X)
			# X is the number of observed features, it's a variable
			X_r = X.reshape(1,X.shape[0], X.shape[1])
			
			y_hat = self.model.predict(X_r)
			print(y_hat.shape)

# 			y_hat = y_hat.reshape((self.n_out, 1))
			...
			real_vals = X_rescaled[:,-1]
			
			predo_vals = [int(i) for i in y_rescaled[:,-1]]
			
			self.p.two_plot(real_vals, predo_vals, "Tiempo", "Bicicletas", "%s: %.2f%%" % (self.model.metrics_names[1], scores[1]*100), self.dir_path + "/plots/" + self.city + "/test/" + str(i), text = "", line_1 = "Real", line_2 = "Prediction")
